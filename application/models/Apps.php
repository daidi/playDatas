...
class AppsModel extends RedisModel {
    ...
    public function getDetailJson($packageName, $language = '') {
        ...
        if ($data = $this->redis->get('appboxD_' . $packageName . '_' . $this->language)) {
            ...
            if ($data) {
                ...
                //if ($data['status'] == 0 || !$data['status']) return json_encode(array('status' => 2));//如果app已经下线
    ...
    public function updateRedis($redisData, $data) {
        //如果评分，下载量，名称有改变，则更新缓存
        if ($redisData['name'] != $data['name'] || $redisData['downloadCount'] != $data['downloadCount'] || $redisData['rate'] != $data['rate']) {
            $this->redis->select(8);
