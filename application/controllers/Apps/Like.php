...
class Apps_LikeController extends Yaf_Controller_Abstract 
{
    public function likeAction() 
    {
        ...
        if($cancel == 1)
            $sql = "update appbox_app set likeCount=likeCount-1 where package_name='$packageName' and language='$language'";
        else
            $sql = "update appbox_app set likeCount=likeCount+1 where package_name='$packageName' and language='$language'";

        $other_mod = new OtherModel($language);
        $json = $other_mod->handleLike($sql,$packageName);
    ...
    public function hateAction()
    {
        ...
        if($cancel)
            $sql = "update appbox_app set hateCount=hateCount-1 where package_name='$packageName' and language='$language'";
        else
            $sql = "update appbox_app set hateCount=hateCount+1 where package_name='$packageName' and language='$language'";
